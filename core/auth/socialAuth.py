...
class GoogleAuthenticationCallbackView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user_info_response.status_code != 200:
            messages.error(
                request, _('The user info response is not found or is invalid')
            )
            return redirect('core:login')

        user_info_response.json().get('given_name')
        user_info_response.json().get('family_name')
        ...
        if not get_user_model().objects.filter(email=email).exists():
            ...
            if not user.is_active:
                messages.error(
                    request,
                    _(
                        'Your account is not verified. Please check your email to verify your email address.'
                    ),
                )
                return redirect('core:login')
            else:
...
def email_verification_link(request, user):
    ...
    html_message = render_to_string(
        'core/auth/email/email_verification.html', context
    )
    plain_message = render_to_string(
        'core/auth/email/email_verification.txt', context
    )
    msg = EmailMultiAlternatives(
        subject, plain_message, settings.FROM_DEFAULT_EMAIL, [user.email]
    )
